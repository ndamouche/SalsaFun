...
let printFInterval x = match x with
  (I(a,b)) -> if a = b   then "["^(string_of_float b)^"]" 
...
let printRInterval x = match x with
  (J(a,b)) -> if a = b   then "["^(string_of_float b)^"]" 
...
let fCos x = match x with 
    (I(a,b)) -> if ((b-.a)>(6.283)) then (I((-1.0),1.0))
                else 
                  let a' = min (cos a) (cos b) in
                  let b' = max (cos a) (cos b) in
				  let a'' = if (((mod_float a 6.283)<=3.1416) && ((mod_float b 6.283)>= 3.141)) then (-1.0) else a' in
				  let b'' = if (((mod_float a 6.283)<=0.0) && ((mod_float b 6.283)>= 0.0)) then (1.0) else b' in
				  let b''' = if ((mod_float a 6.283)> (mod_float b 6.283)) then (1.0) else b'' in
                    (I(a'',b'''))
| _ -> Empty
;;


let fSin x = fCos (fPlus x (I(1.57,1.58))) ;;


let fExp x = match x with 
    (I(a,b)) -> I(exp a,exp b)
| _ -> Empty
;;

let fLog x = match x with 
    (I(a,b)) -> I(log a,log b)
| _ -> Empty
;;


let fIntOfBool x = match x with 
 (I(a,b)) -> I( a, b)
| _ 	  -> Empty
;;
						  

let fBoolOfInt x = match x with 
  (I(a,b))  -> I(a,b)
| _ 	    -> Empty
;;
...
let fMeet x y = match (x,y) with
  (I(a,b),I(c,d)) -> let (u,v) = (max a c,min b d) in
...
let fUlp x y =   (* computes half-ulp *)
	             let z = (max (abs_float x) (abs_float y)) in
		     let u = z *. (2.0 ** (-53.)) in u ;;

let ulpAux x y = (* computes half-ulp *)
	             let z = (max (abs_float x) (abs_float y)) in
		     let u = 2.0 ** (-53.) in
                     let mu = u *. -1.0 in  
                       J(mu *. 0.1,u *. 0.1) ;;
...
let ulpAuxTrigo x y = (* computes half-ulp *)
	             let z = (max (abs_float x) (abs_float y)) in
				 let u = z *. 2.0 ** (-20.) in 
                   J(0.0 -. u,u) ;;
...
let rPlus  x y = match (x,y) with
  ((J(v0Min,v0Max)),(J(v1Min,v1Max))) -> let a = v0Min +. v1Min in
                         		 		 let b = v0Max +. v1Max in 
										 let u = 2. *. (fUlp a b) in J(a -. u,b +. u)
| (_,JInfty) -> JInfty
| (JInfty,_) -> JInfty
| _ -> JEmpty
;;
...
let rMult x y = match (x,y) with
  ((J(v0Min,v0Max)),(J(v1Min,v1Max))) -> let f1 = (v0Min *. v1Min) in
                                         let f2 = (v0Min *. v1Max) in
                                         let f3 = (v0Max *. v1Min) in
                                         let f4 = (v0Max *. v1Max) in 
										 let a = fmin4 f1 f2 f3 f4 in
										 let b = fmax4 f1 f2 f3 f4 in
										 let u = 2. *. (fUlp a b) in J(a -. u,b +. u)
| (_,JInfty) -> JInfty
| (JInfty,_) -> JInfty
| _ -> JEmpty
...
let rSqrt x = match x with 
  (J(v0Min,v0Max)) -> 	let a = sqrt v0Min in
					 	let b = sqrt v0Max in
						let u = 2. *. (fUlp a b) in J(a -. u,b +. u)
| JInfty -> JInfty
| _ -> JEmpty
...
let rCos x = match x with 
    (J(a,b)) -> if ((b-.a)>(6.283)) then (J(-1.0,1.0))
                else
                  let a' = min (cos a) (cos b) in
                  let b' = max (cos a) (cos b) in
				  let a'' = if (((mod_float a 6.283)<=3.1416) && ((mod_float b 6.283)>= 3.141)) then (-1.0) else a' in
...
let rSin x = rCos (rPlus x (J(1.57,1.58))) ;;


let rExp x = match x with 
    (J(a,b)) -> J(exp a,exp b)
| _ -> JEmpty
;;

let rLog x = match x with 
    (J(a,b)) -> J(log a,log b)
| _ -> JEmpty
;;


let rIntOfBool x = match x with 
 (J(a,b)) -> J( a, b)
| _ -> JEmpty
;;


let rBoolOfInt x = match x with 
 (J(a,b)) -> J( a, b)
| _ -> JEmpty
;;
...
let r_of_f x = match x with
  I(a,b) -> J(a,b)
| Empty -> JEmpty ;;


let f_of_r x = match x with
  J(a,b) -> I(a,b)
| JEmpty -> Empty 
| JInfty -> I(neg_infinity,infinity) ;;
...
let feUMinus (f1,e1) = feMinus (I(0.0,0.0),J(0.0,0.0)) (f1,e1) ;;
...
let feDiv (f1,e1) (f2,e2) =  let f = fDiv f1 f2 in
                             let u = ulp f in 
...
let feSqrt (f1,e1) = 	let f = fSqrt f1 in
			...
			let o1 = rDiv ef1 (J(2.0,2.0)) in
			let o2 = rDiv ef2 (J(8.0,8.0)) in
			let o3 = rDiv ef3 (J(16.0,16.0)) in
			let o4 = rDiv ef4 (J(128.0,128.0)) in
...
let feCos s = match s with
 (f,e) ->  let f' = fCos f in 
	   let e' = match e with
	     J(a,b) -> rMinus (rPlus (rMult (rMinus (rCos (r_of_f f)) (J(1.0,1.0))) 
                              (rMinus (rCos e) (J(1.0,1.0))))
                              (rMinus (rCos e) (J(1.0,1.0))))
			      (rMult (rSin (r_of_f f)) (rSin e)) 
	    |	_ -> JInfty
		in (f',e') 
;;


let feSin s = match s with
  (f,e) ->  let f' = fSin f in 
	    let e' = match e with
	      J(a,b) -> rPlus (rMult (rSin (r_of_f f))
              (rMinus (rCos e) (J(1.0,1.0))))
              (rPlus (rMult (rMinus (rCos (r_of_f f)) (J(1.0,1.0))) (rSin e)) (rSin e))
	      |	_ -> JInfty
		in (f',e') ;;


let feExp s = match s with
  (f,e) ->  let f' = fExp f in 
	    let e' = match e with
              J(a,b) -> rPlus (rMult (rMinus (rExp e) (J(1.0,1.0))) 
              (rMinus (rExp (r_of_f f)) (J(1.0,1.0))) )
              (rMinus (rExp e) (J(1.0,1.0)))
	    | _ -> JInfty
		in (f',e') ;;


let feLog s = match s with
  (f,e) -> let f' = fLog f in 
	   let e' = match e with
	     J(a,b) -> rDiv e (r_of_f f) 
	     |	_ -> JInfty
		in (f',e') ;;

let feIntOfBool be = if (be = 1) then (I(1.0,1.0),J(0.0,0.0)) else (I(-1.0,-1.0),J(0.0,0.0));; 
 

let feBoolOfInt be = if (be = 0) then false else true;; 
 

let feIntOfBool2 be = match be with 
  BCst(be,_)  -> if (be) then (I(1.0,1.0),J(0.0,0.0)) else (I(-1.0,-1.0),J(0.0,0.0))
| _           -> raise (Error "Case not yet implemented : IntOfBool2") 
;; 


let feJoin (a,b) (c,d)  = (fJoin a c,rJoin b d) ;;

  
let feMeet (a,b) (c,d)  = (fMeet a c,rMeet b d) ;;
...
let feLte (f,e) (f',e') = match (f,f') with (* 1 = true, -1 = false, 0 = ? *)
  (I(a,b),I(c,d)) -> if ((b < c) || ((a=b) && (b=c) && (c=d))) then 1 else 
                     if (d < a) then (-1) else 0 
| _ -> 0 ;;
...
let feGte (f,e) (f',e') = match (f,f') with (* 1 = true, -1 = false, 0 = ? *)
  (I(a,b),I(c,d)) -> if ((d < a) || ((a=b) && (b=c) && (c=d))) then 1 else 
                     if (b < c) then (-1) else 0  
| _ 		  -> 0 
;;
...
let feAnd v1 v2 = if (( v1 = 1 ) && (v2 = 1)) then 1 else 
		  if (( v1 = -1 ) || (v2 = -1)) then -1 else 0 
;; 

let feOr v1 v2 = if (( v1 =1) || (v2 = 1)) then 1 else
		 if (( v1 = -1) && (v2 = -1)) then -1 else 0 
;;
...
let errLt (f,e) (f',e') = match (e,e') with
  (J(a,b),J(c,d)) -> if ((max (abs_float a) (abs_float b)) < (max (abs_float c) (abs_float d))) then (-1) else (1) 
| (J(_),JEmpty) -> 0
| (JEmpty,J(_)) -> 0
| (JEmpty,JEmpty) -> 0 
| (JInfty,JInfty) -> 0
...
| Cos(e1,_)      -> let v1 = execExpr e1 env in feCos v1
| Sin(e1,_)      -> let v1 = execExpr e1 env in feSin v1
| Exp(e1,_)      -> let v1 = execExpr e1 env in feExp v1
| Log(e1,_)      -> let v1 = execExpr e1 env in feLog v1
| IntOfBool(be,_)-> let v1 = execTest be env in feIntOfBool v1


and execTest be env = match be with
  BCst(b,_)      -> if b then 1 else -1 
| Lt(e1,e2,_)    -> let v1 = execExpr e1 env in
                    let v2 = execExpr e2 env in feLt v1 v2
| Lte(e1,e2,_)   -> let v1 = execExpr e1 env in
		    let v2 = execExpr e2 env in feLte v1 v2 
| Gt(e1,e2,_)    -> let v1 = execExpr e1 env in
                    let v2 = execExpr e2 env in feGt v1 v2
| Gte(e1,e2,_)   -> let v1 = execExpr e1 env in 
		    let v2 = execExpr e2 env in feGte v1 v2  
| Eq(e1,e2,_)    -> let v1 = execExpr e1 env in
                    let v2 = execExpr e2 env in feEq v1 v2 
| And(e1,e2,_)   -> let v1 = execTest e1 env in
		    let v2 = execTest e2 env in feAnd v1 v2 
| Or(e1,e2,_)    -> let v1 = execTest e1 env in
		    let v2 = execTest e2 env in feOr v1 v2 
| Not(e1,_)      -> let v1 = execTest e1 env in feNot v1 
| BoolOfInt(e1,_)-> let v1 = execExpr e1 env in if (v1 =  (I(1.0,1.0),J(0.0,0.0)))  then 1 else 0
;;
...
let rec envMeet env1 env2 = match env1 with
  [] -> []
| (id,v)::env1' -> try 
			let v' = getEnv id env2
		        in envMeet env1' (setEnv id (feMeet v v') env2) 
		   with
		     _ -> envMeet env1' env2 
;;
...
| Sqrt(x,_) 		-> let (f1,e1) = evalExprExactAux x env in 
			   let f = rSqrt f1 in
		           let ef1 = rDiv e1 f1 in
		           let ef2 = rMult ef1 ef1 in
		           let ef3 = rMult ef1 ef2 in
		           let ef4 = rMult ef2 ef2 in
		           let o1 = rDiv ef1 (J(2.0,2.0)) in
		           ...
		           let e'' = rMult e' f 
		             in (f,e'') 
| Cos(x,_) 		-> let (f,e) = evalExprExactAux x env in 
		           let f' = rCos f in
		           let e' = rMinus (rPlus (rMult (rMinus (rCos f) (J(1.0,1.0))) 
                                           (rMinus (rCos e) (J(1.0,1.0))))
                                           (rMinus (rCos e) (J(1.0,1.0))))
				           (rMult (rSin f) (rSin e))
                             in (f',e') 
| Sin(x,_) 		-> let (f,e) = evalExprExactAux x env in 
		           let f' = rSin f in
		           let e' = rPlus (rMult (rSin f)(rMinus (rCos e) (J(1.0,1.0))))
                                          (rPlus (rMult (rMinus (rCos f) (J(1.0,1.0))) (rSin e)) (rSin e))
                             in (f',e') 
| Exp(x,_) 		-> let (f,e) = evalExprExactAux x env in 
		           let f' = rCos f in
		           let e' = rPlus (rMult (rMinus (rExp e) (J(1.0,1.0))) 
                                          (rMinus (rExp f) (J(1.0,1.0))))
                                          (rMinus (rExp e) (J(1.0,1.0)))
                             in (f',e') 
| Log(x,_) 		-> let (f,e) = evalExprExactAux x env in 
		           let f' = rSin f in
		           let e' = rDiv e f
                             in (f',e') 

| IntOfBool(BCst(be,_),_)  -> if be then ((J(1.0,1.0),J(0.0,0.0))) else ((J(-1.0,-1.0),J(0.0,0.0))) 

| _       -> raise(Error "case not yet implemented : evalExprExactAux") 
;;
...
| Cos(e1,_) 			-> let v1 = evalExpr e1 env in feCos v1  
| Sin(e1,_) 			-> let v1 = evalExpr e1 env in feSin v1  
| Exp(e1,_) 			-> let v1 = evalExpr e1 env in feExp v1  
| Log(e1,_) 			-> let v1 = evalExpr e1 env in feLog v1  
| IntOfBool(BCst(be,_),_)       -> if be then ((I(1.0,1.0),J(1.0,1.0))) else ((I(-1.0,-1.0),J(1.0,1.0))) 
| FunCall(name, el, _)          -> let f = getFunc name !funList in 
                                   let vl = evalArgsList el env in 
                                       evalFunc f vl env 
| _     			-> raise (Error "case not yet implemented : evalExpr") 

and evalBoolExpression b env = match b with  
...
| Lt(Cst(v,_),Id(x,_),_)	-> let (fx,ex) = getEnv x env in
				   ...
				   let envFalse = setEnv x (resFalse,ex) env in
				     (envTrue,envFalse)	
| Lte(Id(x,_),Cst(v,_),_)	-> let (fx,ex) = getEnv x env in
				   let (c,d) = getI v in
				   let resTrue = fMeet fx (I(neg_infinity,d)) in
				   let resFalse = fMeet fx (I(c,infinity)) in 
				   let envTrue = setEnv x (resTrue,ex) env in
				   let envFalse = setEnv x (resFalse,ex) env in
				     (envTrue,envFalse)
| Lte(Cst(v,_),Id(x,_),_)	-> let (fx,ex) = getEnv x env in
				   let (c,d) = getI v in
				   let resTrue = fMeet fx (I(c,infinity)) in
				   let resFalse = fMeet fx (I(neg_infinity,d)) in
				   let envTrue = setEnv x (resTrue,ex) env in
...
| Gte(e1,e2,l) 			-> evalBoolExpression (Lte(e2,e1,l)) env  
| And(e1,e2,_) 			-> let (envTrue1,envFalse1) = evalBoolExpression e1 env in
			           let (envTrue2,envFalse2) = evalBoolExpression e2 env in
                                    (envMeet envTrue1 envTrue2, envJoin envFalse1 envFalse2)   
| Or(e1,e2,_) 			-> let (envTrue1,envFalse1) = evalBoolExpression e1 env in
			           let (envTrue2,envFalse2) = evalBoolExpression e2 env in
                                    (envJoin envTrue1 envTrue2, envMeet envFalse1 envFalse2)   

| Not(e1,_) 			-> let (envTrue1,envFalse1) = evalBoolExpression e1 env in (envFalse1,envTrue1) 
| BoolOfInt(e1,_)               -> (env,env)    
| _ 				-> raise (Error ("evalBoolExpression: case not yet implemented :-((("))



and  buildFuncEnv args el env = match args with 
  []      -> (match el with 
             []   -> env 
             | _  -> raise (Error ("BuildFuncEnv : Too Many Arguments in Call to  ... guess it ;-))"))
             )
|  x::xs  -> (match el with 
             []      ->  raise (Error ("BuildFuncEnv : Too Few Arguments in Call to ... guess it ;-))"))
             | e::es ->  (buildFuncEnv xs es (setEnv x e env))
             )


and   evalFunc f el env  = match f with 
 Func(name, args, c, return, lab) ->  let env' = buildFuncEnv args el env in 
                                      let env'' = evalCommand c env' in 
                                       (getEnv return env'')  


and evalArgsList el env = match el with 
  []    -> []
| e::es -> (evalExpr e env)::(evalArgsList es env) 


and evalCommand c env = match c with 
  Nop(_)		-> env 
| Assign(id,e,_)	-> let v = evalExpr e env  in setEnv id v env	
...
and exactFixPoint b c env phis = 
  (* let _ = try (print_string ((printCst (getEnv "x@1" env))^" "^(printCst (getEnv "y@1" env))^"\n")) with _ -> () in *) 
...
| Cos(Cst(c1,_),l)              -> let v = evalExprExact e (exprEnv2ValEnv env)
                                     in Cst(v,l)
| Cos(e1,l) 			-> Cos(evalPartExpr e1 env bl,l)
| Sin(Cst(c1,_),l)   	        -> let v = evalExprExact e (exprEnv2ValEnv env)
                                     in Cst(v,l)
| Sin(e1,l) 			-> Sin(evalPartExpr e1 env bl,l)
| Exp(Cst(c1,_),l) 		-> let v = evalExprExact e (exprEnv2ValEnv env)
                                     in Cst(v,l)
| Exp(e1,l) 			-> Exp(evalPartExpr e1 env bl,l)
| Log(Cst(c1,_),l) 		-> let v = evalExprExact e (exprEnv2ValEnv env)
                                     in Cst(v,l)
| Log(e1,l) 			-> Log(evalPartExpr e1 env bl,l) 
| IntOfBool(be1,l) 		-> IntOfBool(evalPartExprBool be1 env bl,l) 


and evalPartExprBool e env bl = match e with
  BCst(_)      -> e
| Lt(e1,e2,l)  -> Lt(evalPartExpr  e1 env bl,evalPartExpr e2 env bl,l)
| Lte(e1,e2,l) -> Lte(evalPartExpr e1 env bl,evalPartExpr e2 env bl,l)
| Gt(e1,e2,l)  -> Gt(evalPartExpr  e1 env bl,evalPartExpr e2 env bl,l)
| Gte(e1,e2,l) -> Gte(evalPartExpr e1 env bl,evalPartExpr e2 env bl,l)
| Eq(e1,e2,l)  -> Eq(evalPartExpr  e1 env bl,evalPartExpr e2 env bl,l)
| And(e1,e2,l) -> And(evalPartExprBool e1 env bl,evalPartExprBool e2 env bl,l)
| Or(e1,e2,l)  -> Or(evalPartExprBool  e1 env bl,evalPartExprBool e2 env bl,l)
| Not(e1,l)    -> Not(evalPartExprBool e1 env bl,l)
| BoolOfInt(e1,l)-> BoolOfInt(evalPartExpr e1 env bl,l) 
;;
...
let rec cleanTmpVarsExpr e exprIdEnv = if (isDefE e exprIdEnv) then
	...
	(match e with
	 ...
	 | Cos(e1,l)      -> Cos(cleanTmpVarsExpr e1 exprIdEnv,l)
	 | Sin(e1,l)      -> Sin(cleanTmpVarsExpr e1 exprIdEnv,l)
	 | Log(e1,l) 	  -> Log(cleanTmpVarsExpr e1 exprIdEnv,l) 
	 | Exp(e1,l)      -> Exp(cleanTmpVarsExpr e1 exprIdEnv,l) 
	 | IntOfBool(be1,l)-> IntOfBool(cleanTmpVarsBoolExpr be1 exprIdEnv,l) 
    )


and cleanTmpVarsBoolExpr e exprIdEnv = match e with
  BCst(_)      -> e
| Lt(e1,e2,l)  -> Lt(cleanTmpVarsExpr  e1 exprIdEnv,cleanTmpVarsExpr e2 exprIdEnv,l)
| Lte(e1,e2,l) -> Lte(cleanTmpVarsExpr e1 exprIdEnv,cleanTmpVarsExpr e2 exprIdEnv,l)
| Gt(e1,e2,l)  -> Gt(cleanTmpVarsExpr  e1 exprIdEnv,cleanTmpVarsExpr e2 exprIdEnv,l)
| Gte(e1,e2,l) -> Gte(cleanTmpVarsExpr e1 exprIdEnv,cleanTmpVarsExpr e2 exprIdEnv,l)
| Eq(e1,e2,l)  -> Eq(cleanTmpVarsExpr  e1 exprIdEnv,cleanTmpVarsExpr e2 exprIdEnv,l)
| And(e1,e2,l) -> And(cleanTmpVarsBoolExpr e1 exprIdEnv,cleanTmpVarsBoolExpr e2 exprIdEnv,l)
| Or(e1,e2,l)  -> Or(cleanTmpVarsBoolExpr  e1 exprIdEnv,cleanTmpVarsBoolExpr e2 exprIdEnv,l)
| Not(e1,l)    -> Not(cleanTmpVarsBoolExpr e1 exprIdEnv,l)
| BoolOfInt(e1,l) -> BoolOfInt(cleanTmpVarsExpr e1 exprIdEnv,l) 
;;


let rec cleanTmpVars c exprIdEnv varRef bl = match c with
  Assign(id,e,l)       -> if (   (not (memberString id bl)) 
                             && (isDefE e exprIdEnv) 
                             && ((String.compare id varRef)!=0)
                             ) 
                          then 
...
| Nop(_)               -> (c,exprIdEnv)
| Seq(c1,c2,l)         -> let (c1',env') = cleanTmpVars c1 exprIdEnv varRef bl in
			  let (c2',env'') = cleanTmpVars c2 env' varRef bl in (Seq(c1',c2',l),env'')
| Cond(b,c1,c2,phis,l) -> let (c1',env') = cleanTmpVars c1 exprIdEnv varRef (bl@(getPhiVars phis)) in
			  let (c2',env'') = cleanTmpVars c2 env' varRef (bl@(getPhiVars phis)) in
			      (* let b' = cleanTmpVarsBoolExpr b exprIdEnv in *)
			      (Cond(b,c1',c2',phis,l),env'')
| While(b,c1,phis,l)   -> let (c1',env') = cleanTmpVars c1 exprIdEnv varRef (bl@(getWPhiBlackList phis)) in
			  (* let b' = cleanTmpVarsBoolExpr b exprIdEnv in *)
...
| Cos(e1,l)      -> (readVars e1)
| Sin(e1,l)      -> (readVars e1)
| Log(e1,l) 	 -> (readVars e1) 
| Exp(e1,l) 	 -> (readVars e1) 
| IntOfBool(be1,l)-> (readVarsBool be1) 


and readVarsBool b = match b with
  BCst(_)      -> []
| Lt(e1,e2,_)  -> (readVars e1)@(readVars e2)
| Lte(e1,e2,_) -> (readVars e1)@(readVars e2)
| Gt(e1,e2,_)  -> (readVars e1)@(readVars e2)
| Gte(e1,e2,_) -> (readVars e1)@(readVars e2)
| Eq(e1,e2,_)  -> (readVars e1)@(readVars e2)
| And(e1,e2,_) -> (readVarsBool e1)@(readVarsBool e2) 
| Or(e1,e2,_)  -> (readVarsBool e1)@(readVarsBool e2) 
| Not(e1,_)    -> (readVarsBool e1)
| BoolOfInt(e1,_)-> (readVars e1) 
;;
